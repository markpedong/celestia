    ...
    <ul className='space-y-2'>
      {sorted.map(({ tag, count }) => (
        <li key={tag.slug}>
          ...
          >
            <span className='truncate font-medium' style={{ color: tag.hashColor }}>
              #{tag.label}
            </span>
            <span className='shrink-0 text-xs text-muted-foreground'>{formatCount(count)}</span>
          </Link>
        </li>
      ))}
    </ul>
