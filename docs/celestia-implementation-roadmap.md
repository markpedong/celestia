...
**P0 is a release gate, not proof of deployment acceptance.** The latest working tree passes typecheck/lint/40 pure tests/build. Local lease fencing, a legacy backup-code factor-reset flow, non-spoofable default limiter keys, a disposable-DB test harness, a read-only staging probe and optional Vercel hourly cron have been added. Migrations, real Supabase/DB/browser acceptance, username enumeration, operational alerts and deployed cron remain open; see `docs/p0-staging-runbook.md`.
...
|---|---|---|---|
| P0.1 | **PARTIAL**: Per-account lease implemented; real recovery gate outstanding | `prisma/migrations/202609250001_pending_account_deletions/`, `prisma/migrations/202609250002_account_deletion_lease/`, `lib/account-deletion.ts`, `lib/server/account-deletion.ts`, `lib/auth.ts`, `vercel.json`, `tests/deletion-lease.test.ts`, `tests/integration/account-deletion.integration.test.ts`; depends on dedicated staging PostgreSQL/Supabase and reviewed migrations | Run the opt-in real-DB test and the disposable Supabase failure matrix. Verify claim exclusivity, ambiguous 404 safety, post-Auth crash, transaction rollback, storage retry, in-flight writes, long-running leases, ownership of retained community media, cron+CRON_SECRET deployment and alerts. Never run destructive production tests. |
| P0.2 | **PARTIAL**: Legacy backup-code reset implemented, provider acceptance missing | `lib/backup-code-recovery.ts`, `lib/actions/security.ts`, `hooks/use-auth-form.ts`, `lib/auth.ts`, `tests/backup-code-recovery.test.ts`; depends on disposable Supabase Auth staging account, Redis and enabled admin factor-reset permissions | 5 helper tests pass. Verify real multi-factor reset, revoked sessions, sign-in/re-enrollment, one-time code invalidation, admin partial failures, stale-code cleanup, AAL2 protection and Redis fail-close. No legacy backup code is represented as an AAL2 session upgrade. |
| P0.3 | **PARTIAL**: Global/account throttles implemented; resolve email enumeration | `lib/username-signin.ts`, `app/api/auth/username/route.ts`, `lib/server/rate-limit.ts`, `services/index.ts`, `hooks/use-auth-form.ts`, `tests/username-signin.test.ts`; depends on trusted ingress and validated server-side credential verification design | 5 limiter tests pass; no arbitrary forwarded header trusted. Replace username-to-email public response with secure credential exchange without disrupting client Supabase session/MFA flow, validate endpoint-level Redis outage and ingress spoofing, then staging sign-in tests. |
...
- Current recorded verification: `pnpm typecheck` PASS; `pnpm lint` PASS; `pnpm test` **40/40 PASS** with zero skipped; `pnpm build` PASS (39/39 static pages). Dedicated-DB integration test added but **1/1 skipped** without explicit staging credentials; staging probe skipped for absent staging URL; real Supabase, migrations, browser E2E, deployed scheduler and alerting remain unverified.

## Next implementation phase

**Next release gate:** execute the opt-in disposable staging DB/Supabase matrix in `docs/p0-staging-runbook.md`, verify both deletion migrations before deploying the claim-aware API, and test native browser MFA recovery plus health/error behavior. Finish secure username credential exchange without disclosing email. Configure monitored cron only after migration/secret readiness. Only then proceed to P1 blocking, moderation, joined feed and cursor pagination. Maintain one row per acceptance feature and reconcile audit counts from the inventory at each milestone. Preserve history instead of marking tests or deployment completed merely because a handler exists.
