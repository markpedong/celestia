...
- **Current provider blocker:** A read-only Supabase Admin API smoke test against the configured main test project returned HTTP **401** from Auth admin listing and **403** from Storage bucket listing. The configured key is in the new `sb_secret_` format, but its authorization against this project is not verified. Re-check that the local `SUPABASE_SERVICE_ROLE_KEY` is an active secret key for the same project as `NEXT_PUBLIC_SUPABASE_URL`; never paste the key into reports or commits. Do not run real Auth/Storage deletion until read-only admin access succeeds.
