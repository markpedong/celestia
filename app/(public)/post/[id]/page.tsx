...
import { isCommunityMember, listCommunity } from '@/lib/db/community.queries';
...
import { CommunityMembershipProvider } from '@/hooks/useQueries';
...
const Page = async ({ params }: PostPageProps) => {
  ...
  const [commentTree, isMember] = await Promise.all([
    getCommentTree(post.id, undefined),
    isCommunityMember(communitySlug, await getCurrentUserID()),
  ]);
  const tagsBySlug = new Map(tags.map(tag => [tag.slug, tag]));

  return (
    <CommunityMembershipProvider memberships={{ [communitySlug]: isMember }}>
      <ContentWithSidebar
