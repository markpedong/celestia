...
import { getCommunityBySlug, isCommunityMember, listCommunity } from '@/lib/db/community.queries';
...
const CommunityPage = async ({ params }: CommunityPageProps) => {
  ...
  const isMember = await isCommunityMember(slug, await getCurrentUserID());

  return (
    <CommunityMembershipProvider memberships={{ [slug]: isMember }}>
