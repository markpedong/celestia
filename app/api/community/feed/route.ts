...
const respondWithFeed = async (slug: unknown, sort: unknown, cache = false) => {
  ...
  if (cache && !sessionUser) {
    const version = await feedCacheVersion();
    const cacheKey = feedCacheKey('community', { slug: community.slug, sort: feedSort, limit: 50, v: version });
    const cached = await getFeedCache<CommunityFeed>(cacheKey);
    if (cached) return NextResponse.json(cached);

    const feed = await getCommunityFeedData(community.slug, feedSort, undefined);
